...
def _generate_samples_with_tc(diffusion_model, config, logger, tokenizer):
    logger.info("Starting Sample Eval.")
    model = _load_from_checkpoint(
        diffusion_model=diffusion_model, config=config, tokenizer=tokenizer
    )
    ...
    for i in range(config.sampling.num_sample_batches):
        if config.sampling.semi_ar:
            ...
            different_in_batch = (
                config.loader.eval_batch_size // config.sampling.duplicate
            )
            ...
            model.metrics.record_tc(
                [
                    i * different_in_batch + j
                    for _ in range(config.sampling.duplicate)
                    for j in range(different_in_batch)
                ],
                samples,
            )
            all_samples.extend(list(text_samples))
...
def generate_reflow_dataset(diffusion_model, config, logger, tokenizer):
    ...
    test_ds = get_pseudo_dataloader(config, tokenizer, model)
...
@torch.inference_mode()
def generate_reflow_dataset_with_perturbed_rect(
    ...
    train_ds, _ = get_dataloaders(config, tokenizer, skip_valid=True)
