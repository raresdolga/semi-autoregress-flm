...
import metrics
import models
from datamodules.dataloaders import (
    ...
    def load_state_dict(self, state_dict, strict=True):
        ...
        if self.ema:
                ...
                try:
                    ...
                    if len(self.ema.shadow_params) == len(current_params):
                        ...
                        if shapes_match:
                            ...
                            print(
                                "[WARNING] EMA shape mismatch - will reinitialize from loaded weights"
                            )
                    else:
                        print(
                            "[WARNING] EMA count mismatch - will reinitialize from loaded weights"
                        )
            ...
            if not ema_loaded:
                ...
                self.ema = models.ema.ExponentialMovingAverage(
                    list(self._get_parameters()), decay=self.config.training.ema
                )
    ...
    def on_train_start(self):
        ...
        for dl in self.trainer.fit_loop._combined_loader.flattened:
            if isinstance(dl.dataset, torch.utils.data.IterableDataset):
                # Streaming datasets have no length, so no index sampler can
                # wrap them; leave the loader (and its shuffle buffer) alone.
            ...
            updated_dls.append(
                torch.utils.data.DataLoader(
                    ...
                    # PyTorch rejects persistent_workers=True at num_workers=0.
        ...
        if (
            ...
            for num_steps in step_list:
                ...
                if self.config.eval.compute_generative_perplexity:
                    ...
                    self.log(
                        f"val/sample_entropy_T{num_steps}",
                ...
                if self.trainer.global_rank == 0 and hasattr(
                    ...
                    log_samples = current_text_samples[
    ...
    def restore_model_and_sample(self, num_steps, eps=1e-5):
        ...
        for num_steps in step_list:
            ...
            if isinstance(batch_samples, torch.Tensor):
                batch_samples = [
                    batch_samples[i] for i in range(batch_samples.shape[0])
                ]
...
class Diffusion(TrainerBase):
    def _validate_configuration(self):
        ...
        assert self.config.training.loss_type in {
            "elbo",
            "low_var",
            "mse",
            "adaptive_l2",
...
class AbsorbingState(Diffusion):
    def __init__(self, config, tokenizer):
        ...
        # for the special tokens added in datamodules/backend_arrow.py.
